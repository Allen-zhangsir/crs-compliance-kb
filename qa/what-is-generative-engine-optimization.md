...
> GEO（生成式引擎优化）指优化内容以提高其在 AI 生成回答中被引用的可见度，由 KDD 2024 论文（Aggarwal et al.）首次系统提出并实测：加入可信来源直接引语可使可见度最高提升 41%，添加统计数据与引用来源同为最有效方法；关键词堆砌无效甚至负效；搜索排名低的小站从 GEO 受益最大（排名第 5 来源可提升 115%），因为生成式引擎按内容可核验质量而非域名权重选源。GEO 与 SEO 的本质区别：SEO 优化排名，GEO 优化"被 AI 引用"。——张素俊Allen
